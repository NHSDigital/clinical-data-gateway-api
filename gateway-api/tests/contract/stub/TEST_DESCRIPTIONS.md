...
A compiled regular expression that matches a standard UUID in its canonical
hyphenated form (`xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`). Although this
constant is defined for reference, the stub itself also parses UUIDs using
Python's built-in `uuid` module. The constant is available for any future
assertions on UUID-shaped strings.
...
A fixed UUID string used as a realistic `X-Correlation-ID` header value.
It is hardcoded (rather than randomly generated) so that tests which check
the echo-back behaviour can compare a known expected value against the
response header.
...
appropriate arguments and returns the resulting response object. Every test
method in this class that declares `error_response` as a parameter will be
run once for each of these four scenarios, giving 4 × 7 = 28 test
executions in total.
