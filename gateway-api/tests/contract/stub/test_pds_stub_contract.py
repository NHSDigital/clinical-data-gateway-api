...
class TestGetPatientNotFound:
    ...
    def test_response_body_resource_type_is_operation_outcome(
        ...
        """Error responses must be FHIR OperationOutcome resources."""
        response = stub.get_patient(
            nhs_number=_UNKNOWN_NHS_NUMBER, request_id=_VALID_REQUEST_ID
        )
        body = response.json()
    ...
    def test_operation_outcome_has_issue(self, stub: PdsFhirApiStub) -> None:
        response = stub.get_patient(
            nhs_number=_UNKNOWN_NHS_NUMBER, request_id=_VALID_REQUEST_ID
        )
        body = response.json()
    ...
    def test_operation_outcome_coding_system(self, stub: PdsFhirApiStub) -> None:
        """The Spine error code system URI must match the spec."""
        response = stub.get_patient(
            nhs_number=_UNKNOWN_NHS_NUMBER, request_id=_VALID_REQUEST_ID
        )
        body = response.json()
    ...
    def test_operation_outcome_issue_severity_is_error(
        self, stub: PdsFhirApiStub
    ) -> None:
        response = stub.get_patient(
            nhs_number=_UNKNOWN_NHS_NUMBER, request_id=_VALID_REQUEST_ID
