...
class TestGetDeviceBundleSuccess:
    ...
    def test_response_bundle_has_at_least_one_entry_for_known_org(
        self, stub: SdsFhirApiStub
    ) -> None:
        """A known seeded organisation must return at least one Device entry."""
        response = stub.get_device_bundle(
            url=_BASE_DEVICE_URL,
            headers={"apikey": "test-key"},
            params={"organization": _ORG_PROVIDER, "identifier": _INTERACTION_ID_PARAM},
    ...
    def test_response_bundle_entry_has_resource(self, stub: SdsFhirApiStub) -> None:
        """Each entry must have a ``resource`` field."""
        response = stub.get_device_bundle(
            url=_BASE_DEVICE_URL,
            headers={"apikey": "test-key"},
            params={"organization": _ORG_PROVIDER, "identifier": _INTERACTION_ID_PARAM},
...
class TestGetEndpointBundleSuccess:
    ...
    def test_response_bundle_has_entries_for_known_interaction_id(
        self, stub: SdsFhirApiStub
    ) -> None:
        """A known seeded interaction ID must return at least one Endpoint entry."""
        response = stub.get_endpoint_bundle(
            url=_BASE_ENDPOINT_URL,
            headers={"apikey": "test-key"},
            params={"identifier": _INTERACTION_ID_PARAM},
...
class TestGetConvenienceMethod:
    ...
    def test_get_device_without_apikey_returns_400(self, stub: SdsFhirApiStub) -> None:
        """Missing ``apikey`` header must yield 400 when routed to /Device."""
        response = stub.get(
            url=_BASE_DEVICE_URL,
            headers={},
            params={"organization": _ORG_PROVIDER, "identifier": _INTERACTION_ID_PARAM},
    ...
    def test_get_endpoint_without_apikey_returns_400(
        self, stub: SdsFhirApiStub
    ) -> None:
        """Missing ``apikey`` header must yield 400 when routed to /Endpoint."""
        response = stub.get(
            url=_BASE_ENDPOINT_URL,
            headers={},
            params={"identifier": _INTERACTION_ID_PARAM},
